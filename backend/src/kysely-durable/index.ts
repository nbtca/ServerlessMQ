...
class D1Connection implements DatabaseConnection {
	...
	async executeQuery<O>(compiledQuery: CompiledQuery): Promise<QueryResult<O>> {
		...
		const numAffectedRows =
			results.rowsWritten > 0 ? BigInt(results.rowsWritten) : undefined;
		return {
			// insertId:
	...
	// biome-ignore lint/correctness/useYield: Kysely requires this method
	async *streamQuery<O>(
